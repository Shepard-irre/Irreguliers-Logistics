...
def render_user_form(user_data=None):
    """Render form to create or edit user"""
    col1, col2 = st.columns(2)
    with col1:
        username = st.text_input("Nom d'utilisateur", value=user_data.get("username", "") if user_data else "")
    with col2:
        email = st.text_input("Email", value=user_data.get("email", "") if user_data else "")

    if not user_data:  # Creating new user
        password = st.text_input("Mot de passe", type="password")
        confirm_password = st.text_input("Confirmer mot de passe", type="password")
    else:
        password = None
        confirm_password = None
        change_pwd = st.checkbox("Changer le mot de passe")
        if change_pwd:
            password = st.text_input("Nouveau mot de passe", type="password")
            confirm_password = st.text_input("Confirmer mot de passe", type="password")

    # Role selection
    all_roles = uex.get_all_roles()
    role_options = {r['name']: r['id'] for r in all_roles}
    current_roles = [r['id'] for r in user_data['roles']] if user_data else []

    selected_role_ids = []
    st.write("**Rôles**")
    cols = st.columns(3)
    for idx, (role_name, role_id) in enumerate(role_options.items()):
        with cols[idx % 3]:
            if st.checkbox(role_name, value=role_id in current_roles, key=f"role_{role_id}"):
                selected_role_ids.append(role_id)

    # Status
    is_active = st.checkbox("Actif", value=user_data.get("is_active", True) if user_data else True)
...
# --- PAGE 8 : GESTION UTILISATEURS (ADMIN PANEL) ---
elif selected_page == "👤 Gestion Utilisateurs":
    st.header("👤 Gestion Utilisateurs & Rôles")

    admin_tabs = st.tabs(["👥 Utilisateurs", "🔐 Rôles"])
    ...
    with admin_tabs[0]:
        ...
        with col2:
            if st.button("➕ Créer Utilisateur", use_container_width=True):
                st.session_state.create_user_form = not st.session_state.get("create_user_form", False)
                st.rerun()

        # Create user form
        if st.session_state.get("create_user_form", False):
            ...
            with col_submit:
                if st.button("Créer", type="primary", use_container_width=True):
                    if not form_data["username"]:
                        st.error("Le nom d'utilisateur est requis")
                    ...
                    elif not form_data["roles"]:
                        st.error("Au moins un rôle doit être assigné")
                    else:
                        # Create user
                        new_user_id = uex.create_user(form_data["username"], form_data["password"], form_data["email"])
                        if new_user_id:
                            # Assign roles
                            for role_id in form_data["roles"]:
                                uex.add_user_role(new_user_id, role_id)
                            st.success(f"✅ Utilisateur {form_data['username']} créé avec succès")
                            st.session_state.create_user_form = False
                            st.rerun()
                        else:
                            st.error(f"❌ Erreur : L'utilisateur {form_data['username']} existe déjà")
            with col_cancel:
        ...
        # Users table
        users = uex.get_all_users()
        st.write(f"**Total: {len(users)} utilisateurs**")

        for idx, user_item in enumerate(users):
            with st.expander(f"👤 {user_item['username']} {'✓ Actif' if user_item['is_active'] else '❌ Inactif'} | Rôles: {', '.join([r['name'] for r in user_item['roles']])}"):
                col1, col2, col3 = st.columns(3)

                with col1:
                    if st.button("✏️ Modifier", use_container_width=True, key=f"btn_edit_user_{idx}"):
                        st.session_state[f"edit_user_{user_item['id']}"] = not st.session_state.get(f"edit_user_{user_item['id']}", False)
                        st.rerun()

                with col2:
                    if st.button("🔄 Changer Actif/Inactif", use_container_width=True, key=f"btn_toggle_user_{idx}"):
                        uex.toggle_user_active(user_item['id'], not user_item['is_active'])
                        st.rerun()

                with col3:
                    if st.button("🗑️ Supprimer", use_container_width=True, key=f"btn_delete_user_{idx}"):
                        uex.delete_user(user_item['id'])
                        st.success(f"✅ Utilisateur {user_item['username']} supprimé")
                        st.rerun()

                # Edit form
                if st.session_state.get(f"edit_user_{user_item['id']}", False):
                    st.info(f"**Modifier {user_item['username']}**")
                    form_data = render_user_form(user_item)

                    col_save, col_cancel_edit = st.columns(2)
                    with col_save:
                        if st.button("Enregistrer", type="primary", use_container_width=True, key=f"save_user_{idx}"):
                            # Update user info
                            uex.update_user(user_item['id'], form_data["username"], form_data["email"])
                            ...
                            if form_data["password"]:
                                if form_data["password"] != form_data["confirm_password"]:
                                    ...
                                    uex.change_password(user_item['id'], form_data["password"])

                            # Update roles
                            current_role_ids = [r['id'] for r in user_item['roles']]
                            for role_id in current_role_ids:
                                if role_id not in form_data["roles"]:
                                    uex.remove_user_role(user_item['id'], role_id)
                            for role_id in form_data["roles"]:
                                if role_id not in current_role_ids:
                                    uex.add_user_role(user_item['id'], role_id)

                            # Update active status
                            uex.toggle_user_active(user_item['id'], form_data["is_active"])

                            st.success("✅ Utilisateur mis à jour")
                            st.session_state[f"edit_user_{user_item['id']}"] = False
                            st.rerun()
                    with col_cancel_edit:
                        if st.button("Annuler", use_container_width=True, key=f"cancel_edit_{idx}"):
                            st.session_state[f"edit_user_{user_item['id']}"] = False
                            st.rerun()
    ...
    with admin_tabs[1]:
        ...
        col1, col2 = st.columns([3, 1])
        with col2:
            if st.button("➕ Créer Rôle", use_container_width=True):
                st.session_state.create_role_form = not st.session_state.get("create_role_form", False)
                st.rerun()

        # Create role form
        if st.session_state.get("create_role_form", False):
            st.info("**Créer un nouveau rôle**")
            role_name = st.text_input("Nom du rôle")
            role_desc = st.text_area("Description")

            # Permission selection
            all_perms = uex.get_default_permissions()
            st.write("**Permissions**")
            cols = st.columns(2)
            selected_perms = []
            for idx, (perm_key, perm_name) in enumerate(all_perms.items()):
                with cols[idx % 2]:
                    if st.checkbox(perm_name, key=f"perm_new_{perm_key}"):
                        selected_perms.append(perm_key)

            col_submit, col_cancel = st.columns(2)
            with col_submit:
                if st.button("Créer Rôle", type="primary", use_container_width=True):
                    if not role_name:
                        st.error("Nom du rôle requis")
                    else:
                        role_id = uex.create_role(role_name, role_desc, selected_perms)
                        if role_id:
                            st.success(f"✅ Rôle {role_name} créé")
                            st.session_state.create_role_form = False
                            st.rerun()
                        else:
                            st.error("❌ Erreur : Ce rôle existe déjà")
            with col_cancel:
                if st.button("Annuler", use_container_width=True):
                    st.session_state.create_role_form = False
                    st.rerun()
        ...
        # Roles table
        roles = uex.get_all_roles()
        st.write(f"**Total: {len(roles)} rôles**")

        for idx, role in enumerate(roles):
            role_type = "🔒 Système" if role['is_system'] else "📝 Custom"
            with st.expander(f"{role_type} | {role['name']} | {len(role['permissions'])} permissions"):
                st.write(f"**Description:** {role['description']}")
                st.write(f"**Permissions:** {', '.join([uex.get_default_permissions().get(p, p) for p in role['permissions']])}")

                if not role['is_system']:
                    col1, col2, col3 = st.columns(3)
                    with col1:
                        if st.button("✏️ Modifier", use_container_width=True, key=f"btn_edit_role_{idx}"):
                            st.session_state[f"edit_role_{role['id']}"] = not st.session_state.get(f"edit_role_{role['id']}", False)
                            st.rerun()
                    with col3:
                        if st.button("🗑️ Supprimer", use_container_width=True, key=f"btn_delete_role_{idx}"):
                            uex.delete_role(role['id'])
                            st.success(f"✅ Rôle {role['name']} supprimé")
                            st.rerun()
